...
ifeq ($(DISTRO), fedora)
	...
	OS_VERSION = 9.4
        ...
        EXTRA_LABELS = --label=com.redhat.component=${DISTRO}-bootc-intel-gaudi
        EXTRA_LABELS := $(EXTRA_LABELS) --label=com.redhat.license_terms="https://www.redhat.com/en/about/red-hat-end-user-license-agreements\#RHELBootcNvidiaCuda"
        EXTRA_LABELS := $(EXTRA_LABELS) --label=io.k8s.description="Bootc for NVIDIA CUDA provides a container runtime for NVIDIA CUDA accelerated workloads"
        EXTRA_LABELS := $(EXTRA_LABELS) --label=io.k8s.display-name="Bootc for NVIDIA CUDA"
        EXTRA_LABELS := $(EXTRA_LABELS) --label=summary="Provides a container runtime for NVIDIA CUDA accelerated workloads"
...
IMAGE_NAME ?= ${DISTRO}-bootc-intel-gaudi
